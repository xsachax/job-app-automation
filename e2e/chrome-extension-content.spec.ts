...
test("AI assistance fills only unresolved required fields after an explicit click", async ({
  page,
}) => {
  await installContentPanel(page, {
    html: `
      <label for="impact">Describe one project that demonstrates your technical impact *</label>
      <textarea id="impact" required maxlength="160"></textarea>
      <label for="optional-note">Optional note</label>
      <textarea id="optional-note"></textarea>
    `,
    profile: {
    ...
    assistResponse: {
      ...
      suggestions: [
        {
          fieldId: "field-1",
          answer: "Built reliable developer tooling for distributed systems.",
          confidence: 0.94,
          reason: "Supported by the saved summary.",
          sourceKeys: ["summary"],
        },
      ],
    },
  ...
  await expect(page.locator("#impact")).toHaveValue("");
  await expect(
    panel.getByText(/Optional\. Sends your saved profile/i),
  ).toBeVisible();
  await panel
    .getByRole("button", { name: "AI assist remaining fields" })
    .click();

  await expect(page.locator("#impact")).toHaveValue(
    "Built reliable developer tooling for distributed systems.",
  );
  await expect(page.locator("#optional-note")).toHaveValue("");
  await expect(panel.getByText(/1 AI-assisted via OpenAI/i)).toBeVisible();
  ...
  const request = await page.evaluate(() => {
    ...
    ).__panelHarness.messages;
    return messages.find(
      (message) => message.type === "JOB_AUTOFILL_REQUEST_ASSISTANCE",
    );
  });
  ...
  expect(String(request?.fields?.[0]?.label)).toMatch(/technical impact/i);
});
